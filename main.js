    ...
    if(i == index){
      ...
      `
          ...
          userhtml.innerHTML += `<div class="DomKennedy">
            ...
            <p>Phone: ${users[i].phone}</p>
            <p>Email: ${users[i].email}<p>
            <button disabled class="edit" onClick="editUser('${i}')">Edit</button><button disabled class="remove" onClick="deleteUser('${i}')">Delete</button>`
        }
    }
}
  ...
  var updatemail= document.getElementById('unput2email').value;
      ...
      if (updatename== ''|| updatelast==''|| updatephone==''|| updatemail==''){
        alert("INCOMPLETE");
          ...
          user[index].last = updatelast;
          user[index].phone = updatephone;
          user[index].email = updatemail;
